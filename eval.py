...
def eval_unet_vs_ynet(testloader, testset, args):
   ...
   #unet_model = UNet(1, n_classes).to(args.device)
    unet_model = get_model("y_net_layer_add", ratio=args.g_ratio, num_classes=n_classes).to(args.device)
    unet_path = path.join(args.pretrained_path, "best_epoch 68__y_net_layer_add.pt")
    #unet_path = path.join(args.pretrained_path, "unet.pt")
    unet_model.load_state_dict(torch.load(unet_path))

    ynet_model = get_model("y_net_layer_add", ratio=args.g_ratio, num_classes=n_classes).to(args.device)
    ynet_path = path.join(args.pretrained_path, "best_epoch 68__y_net_layer_add.pt")
