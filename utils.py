...
def per_class_dice(y_pred, y_true, num_class):
    ...
    for i in range(num_class):
        GT = y_true[:,:,i].view(-1)  #224, 224, 9]) #shape:torch.Size([50176])
        Pred = y_pred[:,:,i].view(-1) #torch.Size([50176])
...
# GT_temp = y_true[j,:,i].reshape(1, 224, 9)
#GT = GT_temp.reshape(-1, 1)
             

def per_class_dice_mask(y_pred, y_true, num_class, batch_number=1):
    avg_dice = 0
    y_pred = y_pred.data.squeeze() #.cpu().numpy()
    y_true = y_true.data.squeeze() #.cpu().numpy()
    dice_all = np.zeros(num_class)
    for j in range(batch_number):  
        for i in range(num_class):
            GT = y_true[j,:,:,i].view(-1) #ytrue torch.Size([2, 224, 224, 9]) #torch.Size([50176])
            Pred = y_pred[j,:,:,i].view(-1)
